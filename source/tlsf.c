...
static char * mp[POOL_NUMBER] = {NULL};         /* Default memory pool. */

/******************************************************************/
size_t init_memory_pool(int index, size_t mem_pool_size, void *mem_pool)
{
/******************************************************************/
    tlsf_t *tlsf;
    bhdr_t *b, *ib;
    ...
    if (tlsf->tlsf_signature == TLSF_SIGNATURE) {
        mp[index] = mem_pool;
        b = GET_NEXT_BLOCK(mp[index], ROUNDUP_SIZE(sizeof(tlsf_t)));
...
/******************************************************************/
void *tlsf_malloc(int index, size_t size)
{
/******************************************************************/
    void *ret;

    TLSF_ACQUIRE_LOCK(&((tlsf_t *)mp[index])->lock);

    ret = malloc_ex(size, mp[index]);

    TLSF_RELEASE_LOCK(&((tlsf_t *)mp[index])->lock);

    return ret;
}

/******************************************************************/
void tlsf_free(int index, void *ptr)
{
/******************************************************************/

    TLSF_ACQUIRE_LOCK(&((tlsf_t *)mp[index])->lock);

    free_ex(ptr, mp[index]);

    TLSF_RELEASE_LOCK(&((tlsf_t *)mp[index])->lock);
}

/******************************************************************/
void *tlsf_realloc(int index, void *ptr, size_t size)
{
/******************************************************************/
    void *ret;
    ...
    ret = realloc_ex(ptr, size, mp[index]);
...
/******************************************************************/
void *tlsf_calloc(int index, size_t nelem, size_t elem_size)
{
/******************************************************************/
    void *ret;
    ...
    ret = calloc_ex(nelem, elem_size, mp[index]);
